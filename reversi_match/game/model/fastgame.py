...
class FastGame():
    def __init__(self):
        self.DIMENSION = 8
        # self.board = np.zeros((self.DIMENSION, DIMENSION), dtype=np.ushort)
        self.board = list()
    ...
    def is_cell_exist(self, i, j):
        ...
        if 0 <= j < self.DIMENSION and 0 <= i < self.DIMENSION:
    ...
    def move(self, i, j):
        ...
        if not self.get_available_moves():
            self.current_player.pass_next = True
    ...
    def is_end_game(self):
        cond1 = self.get_available_moves()
        if not cond1:
            self.change_player()
            cond2 = self.get_available_moves()
            self.change_player()
            if not cond2:
                return True
        return False
